...
export default function MapPage() {
    ...
    const getCategoryIconHtml = (cat: string) => {
        ...
        return `
            <div class="relative flex items-center justify-center w-10 h-10 transform -translate-x-1/2 -translate-y-1/2">
                <div class="absolute inset-0 bg-${severity === 'Critical' ? 'red' : severity === 'High' ? 'orange' : 'yellow'}-500/30 rounded-full animate-ping"></div>
    ...
    const createCustomIcon = () => {
        ...
        return L.divIcon({
            ...
            html: getCategoryIconHtml(category),
            iconSize: [40, 40],
            iconAnchor: [20, 20],
            popupAnchor: [0, -20]
        });
                    ...
                    <div className="grid md:grid-cols-2 gap-6 items-end">
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        ...
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border shadow-sm ${severity === 'Critical' ? 'bg-red-100 text-red-700 border-red-200' :
                                    ...
                                    <p className="text-xl text-brand-foreground/70">{location}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="md:text-right">
                            <div className="inline-block p-4 bg-white/60 backdrop-blur-xl border border-brand-orange/20 rounded-2xl shadow-sm">
                                <p className="text-sm text-brand-foreground/50 uppercase tracking-widest font-bold mb-1">Coordinates</p>
                                    ...
                                    <Popup className="custom-popup">
                                        <div className="p-1">
                                            ...
                                            <p className="text-sm text-gray-700 leading-relaxed mb-2">
                                                Active alert in {location}. {severity === 'Critical' ? 'Immediate evacuation advised.' : 'Monitoring recommended.'}
                                            </p>
                                        </div>
                                    </Popup>
                                </Marker>
                    ...
                    <div className="card p-8 bg-white/70">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                            ...
                            <div className="flex flex-col gap-3 w-full md:w-auto">
                                <Link
                                    href={`/report?location=${encodeURIComponent(location)}&type=${category}&severity=${severity}&lat=${lat}&lng=${lng}`}
